...
# Requires: pip install langchain-chroma langchain-openai
...
def init_vectorstore():
    try:
        vs = Chroma(
            ...
            collection_name="bedienungsanleitungen",
            embedding_function=embeddings
        )
    except KeyError as e:
        if "_type" in str(e):
            # Corrupted or legacy collection config: clear and recreate
    ...
    <div style="display: flex; gap: 0;">
      ...
      <div style="flex: 1 1 33%; max-width: none;" id="settings-panel">
        ...
        <form action="/upload" method="post" enctype="multipart/form-data">
            <input type="file" name="pdf_file" accept=".pdf,.docx" required>
...
def ask():
    ...
    # Semantic Retrieval via LangChain-Chroma
    docs = retriever.get_relevant_documents(question)
    context = "\n\n".join([d.page_content for d in docs])
    print(f"📏 Kontextgröße: {len(context)} Zeichen (LangChain RAG)")
...
"""
    ...
    return jsonify({
        "answer": answer,
        "sources": [(d.metadata.get("source", "unbekannt"), d.page_content) for d in docs]
    })
...
def upload():
    ...
    docs = [
        Document(
            page_content=chunk,
            metadata={"source": f"{file.filename}_chunk_{i}"}
        )
        for i, chunk in enumerate(chunks)
    ]
    # Füge neue Dokumente zum Vektorstore hinzu
...
# (Optionale Alt-Route, jetzt ohne ChromaDB-Funktionalität)
